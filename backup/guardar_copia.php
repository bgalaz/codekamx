...
include_once ($_SERVER['DOCUMENT_ROOT']."/codekamx/funciones/fechas.php");
...
$DirBase="/home/bgalaz/backup";
$command = "mysqldump --opt -h $Servidor -u $Usuario -p $Password ".
           "codekamx > $DirBase/copia.sql";
echo "$command";
//$executa = "$DirBase -h $Servidor -u $Usuario --password=$Password --opt --ignore-table=$BaseDeDatos.tabbackup $BaseDeDatos > $archivo";

$ult_linea=system($command, $resultado);
echo "$ult_linea<br>";
